...
## Submission status and next acceptance

The Chrome Web Store dashboard showed **Pending review** for 0.1.10 on
September 7, 2026. Do not treat submission as approval, publication or evidence
of newcomer usefulness. Initial distribution is intended to be Unlisted under
the publisher name Entrep Technologies.

When review completes, record the actual outcome. If approved, confirm published
status (approval may instead leave the item staged), saved Unlisted visibility
and geographic availability. Obtain the real listing URL and verify a normal
store installation, version, first use and optional reminder settings before
sharing. Do not replace the submitted package merely for repository documentation
or promo-asset updates. If rejected, address the specific reviewer feedback.

For existing unpacked users, follow the installation-identity/recovery cautions in
SUPPORT.md. Do not uninstall old recovery data or infer automatic migration.
Newcomer observations remain a separate acceptance step, using consenting users'
private bookmarks or synthetic data, never exported personal bookmark dumps.
...
Before recommending the release broadly, complete the non-builder walkthrough.
The founder-approved self-paced policy replaces the former numeric calibration
requirement. The initial submission has occurred; future submissions still require
accurate policy/support, owner-reviewed declarations and a tested package.
Do not infer store approval or usefulness from unit tests. No downgrade/uninstall
as an operation-recovery strategy.
